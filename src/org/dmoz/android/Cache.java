...
public class Cache {
	...
	public void onSQLCreated(SQLiteDatabase database) {
		Shared.log("Cache().onSQLCreated();", mDebug);
		...
		database.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS `cache_category` ON `cache` (`cache_category` ASC) ");
	}

	public void update(String item, String data) {
		Shared.log("Cache().update();", mDebug);

		SQLSimple sql = Shared.mSQL;
		String[] values = { item, data };
		sql.query("INSERT OR REPLACE INTO `cache` (`cache_category`, `cache_data`) VALUES (?, ?)", values);
	}

	public ArrayList<ContentValues> get(String item) {
		Shared.log("Cache().get();", mDebug);
